...
const StudentDashboard = () => {
  // Simulation d'identification de l'étudiant
  const [user] = useState({
    id_utilisateur: 1,
    id_etudiant: 1,
    isDelegue: true
  ...
  // Charger les types de demandes et les demandes de l'étudiant
  useEffect(() => {
    fetchTypes();
    fetchDemandes();
  }, [user.id_etudiant]);
  ...
  const fetchTypes = async () => {
    try {
      ...
      if (response.data.length > 0) {
        ...
        const defaultId = firstItem.id_type || firstItem.id || Object.values(firstItem)[0];
        setIdType(defaultId);
  ...
  // Helper pour récupérer dynamiquement le nom du type de demande
  const renderOptionLabel = (type) => {
    return (
      type.nom_type ||
      type.libelle_type ||
      type.libelle ||
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      {/* Notifications */}
      {message && (
        <div style={{ padding: '10px', background: '#d4edda', color: '#155724', marginBottom: '15px', border: '1px solid #c3e6cb' }}>
          {message}
        </div>
      )}
      {error && (
        <div style={{ padding: '10px', background: '#f8d7da', color: '#721c24', marginBottom: '15px', border: '1px solid #f5c6cb' }}>
          {error}
        </div>
      )}
              ...
              <tbody>
                {demandes.map((d) => (
                  <tr key={d.id_demande}>
                    ...
                    <td style={{ border: '1px solid #ddd', padding: '8px' }}>{d.type_libelle || d.nom_type || d.libelle}</td>
          ...
          <form onSubmit={handleCreateDemande} style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '500px' }}>
            <div>
              ...
              >
                ...
                {typesDemandes.map((type, index) => {
                  const typeId = type.id_type || type.id || index + 1;
                  return (
